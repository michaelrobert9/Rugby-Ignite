// Low-level Firestore "database" used by every module in lib/data/*.
//
// This is the one place that talks to Firestore. teams.ts, venues.ts,
// matches.ts, rankings.ts and config.ts only ever call through here, so the
// rest of the app (pages, admin forms, ranking engine) is unaffected by how
...
const SINGLETON_COLLECTIONS = new Set(['config', 'meta']);
