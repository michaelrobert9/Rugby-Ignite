...
export async function generateMetadata(props: PageProps<'/rankings/[year]'>): Promise<Metadata> {
...
export default async function SeasonRankingPage(props: PageProps<'/rankings/[year]'>) {
  const { year } = await props.params;
  ...
  const years = seasonYears(build.seasons).map((y) => ({ year: y, href: `/rankings/${y}` }));
      ...
      <JsonLd
        data={{
          ...
          itemListElement: listRows.map((r, i) => ({
            ...
            position: i + 1,
            name: r.name,
            item: `${SITE}/rankings/${year}#${schoolSlug(r.name)}`,
          })),
        }}
      />
